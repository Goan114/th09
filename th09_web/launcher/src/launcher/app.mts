...
appShellClient = createAppShellClient({
  shouldDeferReload: () => state.launched === true || state.ready === true || !!document.querySelector("dialog[open]") || !!document.querySelector("#transfer:not([hidden])"),
...
const state: LauncherState = {
  game: "th09", hasSelection: false, music: "ogg-stream", ready: false, launched: false, replayViewer: false,
  musicPreferenceExplicit: false,
  musicPreference: "ogg-stream",
  request: 0, pending: new Map(), source: "", sourceIdentity: "", mobileOpen: false,
  product: "th09", options: { ...defaultOptions }, language: "ja", lessMotion: false, runtimeVariant: "normal",
  netplay: {
    url: "",
...
const musicModeLabel = (mode: MusicMode) => state.game === "th09" && mode !== "none" ? "原版音乐（OGG）" : (state.game === "th10" || state.game === "th08" || state.game === "th09") && mode !== "none" ? "原版音乐（无损）" : mode === "ogg-stream" ? t("settings.music.oggStream")
...
function restoreGamePreferences(gameId: GameId, preferenceId: ProductId = gameId) {
  ...
  if (gameId === "th10" || gameId === "th08" || gameId === "th09") { state.options.frameLimit60Enabled = true; if (!localStorage.getItem("eagler-touhou-game-options-v1-"+gameId)) state.options.touchEnabled = matchMedia("(pointer: coarse)").matches; }
...
function syncMusicSelectAvailability(select: HTMLSelectElement, availability = resolveMusicAvailability(musicAvailabilityContext())) {
  if (state.game === "th10" || state.game === "th08" || state.game === "th09") for (const option of Array.from(select.options)) if (option.value.startsWith("ogg")) option.remove();
...
function render() {
  ...
  $("#musicOption").hidden = state.game !== "th10" && state.game !== "th08" && state.game !== "th09" && !musicAvailability.ogg;
  if (state.game === "th10" || state.game === "th08" || state.game === "th09") { const native = musicSelect.querySelector<HTMLOptionElement>('option[value="midi"]'); if (native) native.textContent = state.game === "th09" ? "原版音乐（OGG）" : "原版音乐（无损）"; $("#frameLimitToggle").disabled = true; $("#frameLimitHintText").textContent = "游戏按照原版每秒 60 次更新运行。"; }
  ...
  frameLimitToggle.disabled = state.game === "th10" || state.game === "th08" || state.game === "th09";
  frameLimitToggle.title = "";
  const frameLimitHint = $("#frameLimitHint");
  $("#frameLimitHintText").textContent = (state.game === "th10" || state.game === "th08" || state.game === "th09") ? "游戏按照原版每秒 60 次更新运行。" : t("settings.frameLimitHint");
  if (state.game === "th10" || state.game === "th08" || state.game === "th09") { $("#alwaysHitboxToggle").closest<HTMLElement>(".mobile-option")!.hidden = true; }
...
function prepareMidi() {
  if (state.game === "th10" || state.game === "th08" || state.game === "th09") return;
