...
const root=fileURLToPath(new URL('../',import.meta.url)),workspace=resolve(root,'..'),sdk=resolve(workspace,'tools/emsdk'),out=resolve(root,process.env.TH09_OUTPUT||(release?'artifacts/sdl-release':'artifacts/sdl3')),objects=resolve(out,'objects');mkdirSync(objects,{recursive:true});
